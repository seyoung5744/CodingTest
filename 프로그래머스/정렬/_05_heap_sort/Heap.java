...
public class Heap<E> {
    ...
    private static final int DEFAULT_CAPACITY = 10; // 최소(기본) 용적 크리
    ...
    private void resize(int newCapacity) {
        ...
        // 새 배열에 기존에 있던 배열의 모든 요소들을 모두 복사해준다.
    ...
    /**
     * @param idx   추가할 노도의 인덱스
     * @param value 재배치 할 노드
     */
    private void siftUp(int idx, E target) {
        // comparator 가 존재할 경우 comparator 을 인자로 넘겨준다.
        if (comparator != null) {
            siftUpComparator(idx, target, comparator);
        } else {
            sizeUpComparable(idx, target);
        }
    }
    ...
    @SuppressWarnings("unchecked")
    private void sizeUpComparable(int idx, E target) {
